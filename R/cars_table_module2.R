#' Cars Table Module UI / Event Manager
#'
...
cars_table_module_ui2 <- function(id) {
  ns <- NS(id)
...
#' Cars Table Module Server/ Event Manager
#'
...
cars_table_module2 <- function(input, output, session) {
  ...
  output$car_table <- renderDT({
    ...
    datatable(
      ...
      formatDate(
        columns = c("event_manager_created_at", "event_manager_modified_at"),
      ...
      tryCatch({
        out <- conn %>%
          tbl('mtcars') %>%select(         Event_Type,
                                           ...
                                           EManager_Status,
                                           Idea_Validation_Status,
                                           Emanager_Comments,everything())%>% collect()%>%
          mutate(
            event_manager_created_at = as.POSIXct(event_manager_created_at, tz = "UTC"),
            event_manager_modified_at = as.POSIXct(event_manager_modified_at, tz = "UTC")
          ) %>%rowwise() %>%filter(!is.na(Ranking)) %>%
          arrange(desc(event_manager_modified_at))
      }, error = function(err) {
  ...
  output$car_tabler <- renderDT({
    ...
    datatable(
      ...
      # ),
      selection = "none",
      class = "compact stripe row-border nowrap", filter = "top",
      callback=JS("
      ...
      options = list(
        ...
        columnDefs = list(
          ...
          list(visible=F, targets=c(28:121,126:129)),
          list(targets = 0, orderable = FALSE)
        ),
      ...
      formatDate(columns = c("event_manager_created_at", "event_manager_modified_at"),
                 method = 'toLocaleString')
  ...
  car_tabler2_prep <- reactiveVal(NULL)
  ...
  observeEvent(cars2(), {
    ...
    if (is.null(car_tabler2_prep())) {
      # loading data into the table for the first time, so we render the entire table
      # rather than using a DT proxy
      car_tabler2_prep(out)
      
    } else {
      
      # table has already rendered, so use DT proxy to update the data in the
      # table without rerendering the entire table
      replaceData(car_tabler2_proxy, out, resetPaging = FALSE, rownames = FALSE)
      
    }
  })
  
  output$car_tabler2 <- renderDT({
    req(car_tabler2_prep())
    out <- car_tabler2_prep()
    datatable(
      out,
      rownames = FALSE,
      # colnames = c(
      ...
      options = list(
        ...
        columnDefs = list(
          list(className = "dt-center", targets = "_all"),
          list(visible=F, targets=c(0,10:129)),
          list(targets = 0, orderable = FALSE)
        ),
        #buttons = I('colvis'),
  ...
  car_tabler2_proxy <- DT::dataTableProxy('car_tabler2')
  ###############################################################
  cars3 <- reactive({
    session$userData$mtcars_trigger()
    req(reactiveValuesToList(auth_out)$user)
