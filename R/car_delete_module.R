...
#' @param car_to_delete string - the Event_Type of the Idea to be deleted
...
car_delete_module <- function(input, output, session, modal_title, car_to_delete, modal_trigger) {
  ...
  observeEvent(modal_trigger(), {
    ...
    #req(session$userData$email == 'MCATLE')
  ...
  observeEvent(input$submit_delete, {
    ...
    tryCatch({
      ...
      showToast("success", "Idea Successfully Deleted")
    }, error = function(error) {

      msg <- "Error Deleting Idea"
